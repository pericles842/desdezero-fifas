...
export class VentasComponent {
  ...
  ngOnInit() {
    ...
    this.socketService.listen('notificationEmail').subscribe((data: any) => {
      let text = data.res.error
        ? `Hubo un problema al enviar el correo ${data.res.message.error}`
        : 'Correo entregado exitosamente';
  ...
  validatePay(id: number) {
    this.toastService
      .confirm('Seguro desea aprobar el pago', '')
        ...
        if (res.isConfirmed) {
          ...
          this.payService.validatePay(id).subscribe({
            next: (sales) => {
              ...
              sales.sale.tikes = sales.sale.tikes
                .split(',')
              ...
              this.filterGlobal(
                {
                  target: this.globalFilterInput.nativeElement,
                } as unknown as Event,
                this.dataTable
              );
  ...
  rejectPayment(id: number) {
    this.toastService
      .confirm('Seguro desea rechazar el pago', '')
      .then((res: SweetAlertResult) => {
        if (res.isConfirmed) {
          this.loading = true;
          this.payService.rejectPay(id).subscribe({
            next: (sales) => {
              //trasformamos los tikes de string a un arreglo de numeros
              sales.sale.tikes = sales.sale.tikes
                ? sales.sale.tikes.split(',').map((tike: string) => tike.trim())
                : [];
              const index = this.sales.findIndex((sale) => sale.id === id);
  ...
  winUser(sale: Sales, tike: string) {
    ...
    this.toastService
      .confirm(
        'Seguro desea seleccionar el ganador ' + sale.usuario,
        'Con el ticket ' + tike
      )
      .then((res: SweetAlertResult) => {
        if (res.isConfirmed) {
          ...
          this.rafflesService
            .winUser(0, sale.usuario, sale.telefono, tike, sale.rifa)
            .subscribe({
              next: (res) => {
                this.toastService.success(
  ...
  sendEmail() {
    ...
    this.payService
      .sendEmail(
        this.messageEmail.correo,
        this.messageEmail.subject,
        this.messageEmail.text,
        this.messageEmail.id_payment
      )
      .subscribe({
        next: (email) => {
          this.modalSendEmail = false;

          let index = this.sales.findIndex(
            (sale) => sale.id === this.messageEmail.id_payment
          );
          this.sales[index].estatus = 'rechazado';

          this.toastService.success(
            'Mensaje enviado correctamente',
            this.sales[index].correo
          );
  ...
  openModalWinUser(sale: Sales) {
    if (sale.estatus == 'rechazado' || sale.estatus == 'pendiente') {
      this.toastService.warning(
        'No se puede seleccionar un ganador con un pago rechazado o pendiente',
        'Advertencia'
      );
      return;
    }
  ...
  showToast(
    ...
    message_email: string = 'Correo entregado exitosamente'
  ) {
    this.messageService.add({
      ...
      contentStyleClass:
        ' p-3 gap-3 flex w-full justify-content-between align-items-center',
      life: 6000,
    });
  }
}
